  ...
  try {
    ...
    if (online) {
      ...
      const bienesExistentes = await models.bienes.findAll({
        attributes: ["secuencia", "sbn"],
      });

      // Convertir los bienes existentes a un set de secuencias para una búsqueda rápida
      const secuenciasExistentes = new Set(bienesExistentes.map(b => b.secuencia));
      const sbnExistentes = new Set(bienesExistentes.map(b => b.sbn));
      ...
      const nuevosBienes = externalData?.data?.filter((item) => {
        return !secuenciasExistentes.has(item.SECUENCIA) && !sbnExistentes.has(item.CODIGO_ACTIVO);
      });
      ...
      await models.bienes.bulkCreate(format, { ignoreDuplicates: true });
      return console.log('Bienes actualizados con éxito!');
