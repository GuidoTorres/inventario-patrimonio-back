...
const estadisticasUso = async (req, res) => {
  ...
  const estadisticasTipo = async (req, res) => {
    try {
      ...
      // Filtrar etiquetas y datos donde el valor es mayor que 0
      const filteredLabels = ["Bienes Activos", "Bienes Sobrantes", "Bienes Faltantes"].filter(
        (label, index) => Object.values(conteosBienes)[index] > 0
      );
      const filteredData = Object.values(conteosBienes).filter(
        (value) => value > 0
  ...
  const estadisticasPorEstadoPatrimonial = async (req, res) => {
    try {
      ...
      console.error("Error al obtener los bienes inventariados por estado patrimonial:", error);
      res
        .status(500)
...
module.exports={estadisticasUso, estadisticasTipo, estadisticasPorSede, estadisticasPorEstadoPatrimonial}
