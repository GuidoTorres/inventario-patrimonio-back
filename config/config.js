...
const dbConfigs = {
  ...
  local: {
    database: "inventario_patrimonio5",
    username: "root",
    password: "root",
    host: "localhost",
    dialect: "mysql",
...
const [existingBajas] = await sequelize.query(
  `SELECT sbn 
   FROM bienes23 
   WHERE sbn IN (${snbsParaBaja.map(sbn => `'${sbn}'`).join(',')})
   AND estado_baja = 1`,
  { type: Sequelize.QueryTypes.SELECT }
);
...
const snbsPorActualizar = snbsParaBaja.filter(sbn => !snbsYaMarcados.has(sbn));
...
if (snbsPorActualizar.length > 0) {
  ...
  logConnectionStatus(`Updated estado_baja for SNBs: ${snbsPorActualizar.join(', ')}`);
} else {
